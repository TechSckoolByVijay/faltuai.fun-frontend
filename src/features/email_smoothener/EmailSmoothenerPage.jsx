...
const EmailSmoothenerPage = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-950 via-slate-900 to-black p-6 sm:p-8 text-gray-100">
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="rounded-xl border border-green-500/40 bg-black/70 p-6 shadow-2xl">
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          <div className="rounded-xl border border-green-500/40 bg-black/75 p-5">
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-xl font-semibold text-green-300">Raw Draft Terminal</h2>
              <button
                onClick={loadSample}
                className="text-xs px-3 py-1 rounded-md border border-green-500 text-green-300 hover:bg-green-900/30"
              >
                Load Sample
              </button>
            </div>
            ...
            <div className="rounded-lg border border-slate-700 bg-slate-950 overflow-hidden">
              <div className="px-3 py-2 text-xs border-b border-slate-700 text-slate-400 font-mono">
                email input
              </div>
              <textarea
                value={rawText}
                ...
                className="w-full h-72 p-4 bg-slate-950 text-green-300 font-mono text-sm focus:outline-none"
              />
            </div>
            ...
            <div className="mt-3 flex items-center justify-between text-xs text-slate-400">
              ...
              <button
                ...
                className="px-5 py-2 rounded-md bg-green-500 text-black font-semibold disabled:bg-slate-600 disabled:text-slate-300"
              >
            ...
            {error && (
              <div className="mt-3 rounded-md border border-red-600/60 bg-red-950/60 px-3 py-2 text-sm text-red-200">
                {error}
              </div>
            )}
          </div>
          ...
          <div className="rounded-xl border border-slate-700 bg-slate-900/80 p-5">
            <h2 className="text-xl font-semibold mb-1">Faltu Output</h2>
            <p className="text-sm text-slate-400 mb-4">Three sendable versions + reply-risk vibe check.</p>

            {!result ? (
              <div className="h-[26rem] rounded-lg border border-dashed border-slate-700 flex items-center justify-center text-sm text-slate-500 text-center px-6">
              ...
              <div className="space-y-4 max-h-[35rem] overflow-y-auto pr-1">
                <div className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-slate-300">
                  <span className="font-semibold text-slate-100">Overall Vibe:</span> {result.overall_vibe}
                </div>

                {result.draft_assessment && (
                  <div className="rounded-lg border border-slate-700 bg-slate-900/80 p-4 space-y-3">
                    <div className="flex items-center justify-between gap-3">
                      <h3 className="text-sm font-semibold text-slate-100">Draft Quality Indicators</h3>
                    ...
                    <div className="text-xs text-slate-300 space-y-1">
                      <div>
                        <span className="font-semibold text-slate-100">Tone:</span> {result.draft_assessment.tone_summary}
                      </div>
                      <div className="flex flex-wrap gap-2">
                        <span className={`px-2 py-1 rounded-full ${result.draft_assessment.sounds_friendly ? 'bg-emerald-900/50 text-emerald-200' : 'bg-slate-800 text-slate-300'}`}>
                          Friendly: {result.draft_assessment.sounds_friendly ? 'Yes' : 'No'}
                        </span>
                        <span className={`px-2 py-1 rounded-full ${result.draft_assessment.sounds_aggressive ? 'bg-rose-900/50 text-rose-200' : 'bg-slate-800 text-slate-300'}`}>
